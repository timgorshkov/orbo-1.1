# Документация пользователя Orbo

> CRM участников, событий и управление сообществами в Telegram и MAX

---

## 📚 Содержание

### 1. Начало работы

- [Регистрация и вход](./01-getting-started/registration.md)
- [Подключение Telegram-аккаунта](./01-getting-started/connect-telegram.md)
- [Создание первой организации](./01-getting-started/first-organization.md)

### 2. Telegram-группы и каналы

- [Какие боты и для чего](./02-telegram-groups/bots-overview.md)
- [Как добавить бота в группу](./02-telegram-groups/add-bot-to-group.md)
- [Подключение группы к организации](./02-telegram-groups/connect-groups.md)
- [Подключение Telegram-канала](./02-telegram-groups/connect-channel.md)
- [Импорт истории сообщений](./02-telegram-groups/import-history.md)

### 3. Участники (CRM)

- [Профили участников](./03-participants-crm/profiles.md)
- [AI-анализ интересов и визиток](./03-participants-crm/ai-enrichment.md)
- [Объединение профилей (Telegram + MAX)](./03-participants-crm/merge-profiles.md)
- [Поиск и фильтрация](./03-participants-crm/search-filters.md)
- [Импорт из WhatsApp](./03-participants-crm/whatsapp-import.md)

### 4. События

- [Создание события](./04-events/create-event.md)
- [MiniApp для регистрации](./04-events/miniapp-registration.md)
- [Настройка формы регистрации](./04-events/registration-form.md)
- [Управление оплатами](./04-events/payments.md)
- [Публикация в Telegram и MAX](./04-events/telegram-publish.md)

### 5. Анонсы

- [Создание и отправка анонсов](./05-announcements/announcements.md)

### 6. Заявки

- [Обзор раздела «Заявки»](./06-applications/overview.md)
- [Воронки и статусы](./06-applications/pipelines.md)
- [Формы заявок и MiniApp](./06-applications/forms.md)

### 7. Уведомления

- [Обзор правил уведомлений](./07-notifications/rules-overview.md)
- [AI-детекция негатива](./07-notifications/ai-negative.md)
- [Неотвеченные вопросы](./07-notifications/unanswered.md)
- [Зоны внимания](./07-notifications/attention-zones.md)

### 8. Организация и настройки

- [Настройки организации](./08-organization/settings.md)
- [Добавление администраторов](./08-organization/add-admins.md)
- [Синхронизация команды с Telegram](./08-organization/team-sync.md)
- [Приглашения участников](./08-organization/invites.md)

### 9. Тарифные планы

- [Обзор планов и возможностей](./09-plans/overview.md)
- [Как перейти на платный план](./09-plans/upgrade.md)

### 10. Интеграция с MAX

- [Что такое MAX и как начать](./10-max-integration/overview.md)
- [Подключение MAX-групп](./10-max-integration/connect-groups.md)
- [Объединение профилей Telegram и MAX](./10-max-integration/merge-profiles.md)

### 11. Публичный портал участников

- [Что такое публичный портал](./11-public-portal/overview.md)
- [Доступ для участников](./11-public-portal/participant-access.md)

### 12. База знаний (Материалы)

- [Обзор раздела Материалы](./12-materials/overview.md)
- [Создание контента](./12-materials/create-content.md)

### 13. Приложения

- [Что такое приложения](./13-mini-apps/overview.md)
- [Создание и публикация](./13-mini-apps/create-app.md)

### 14. FAQ

- [Частые вопросы](./14-faq/faq.md)

---

## 🚀 Быстрый старт

### Шаг 1. Регистрация

Перейдите на [my.orbo.ru](https://my.orbo.ru) и создайте аккаунт через email, Google или Яндекс.

### Шаг 2. Создание организации

Укажите название вашего сообщества. Orbo подскажет первый шаг исходя из вашей роли.

### Шаг 3. Подключение группы

Добавьте бота в вашу Telegram- или MAX-группу. Участники начнут появляться автоматически.

### Шаг 4. Создайте событие с MiniApp

Участники регистрируются прямо в Telegram или MAX. Напоминания отправляются автоматически.

### Шаг 5. Знайте своих людей

Карточки участников с историей посещений, активностью и контактами. AI-анализ интересов и переписки доступен прямо с дашборда.

---

## 🤖 Боты Orbo

### Telegram

| Бот | Username | Назначение |
|-----|----------|------------|
| Community Bot | [@orbo_community_bot](https://t.me/orbo_community_bot) | Сбор аналитики из групп, публикация анонсов |
| Assist Bot | [@orbo_assistant_bot](https://t.me/orbo_assistant_bot) | Уведомления, авторизация, напоминания о событиях |
| Event Bot | [@orbo_event_bot](https://t.me/orbo_event_bot) | MiniApp для регистрации на события и заявки |
| Start Bot | [@orbo_start_bot](https://t.me/orbo_start_bot) | Регистрация новых организаций через Telegram |

### MAX

| Бот | Назначение |
|-----|------------|
| Orbo MAX Bot | Сбор аналитики из MAX-групп, публикация анонсов, MiniApp |

---

## 📞 Поддержка

Если что-то не работает или нужна помощь — обратитесь в поддержку через Telegram: [@orbo_support](https://t.me/orbo_support)

---

*Документация актуальна для версии Orbo 1.1*
*Последнее обновление: Март 2026*
