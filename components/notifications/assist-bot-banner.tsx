...
const DISMISS_KEY = 'orbo_assist_bot_banner_dismissed'
...
/**
 * Shows a banner prompting admin/owner to start @orbo_assistant_bot
 * for receiving admin notifications (negative discussions, unanswered questions, etc.)
 * via Telegram personal messages. Dismissable with localStorage persistence.
 */
export default function AssistBotBanner({ orgId, compact = false }: AssistBotBannerProps) {
  ...
  const [dismissed, setDismissed] = useState(() => {
    ...
    return localStorage.getItem(`${DISMISS_KEY}_${orgId}`) === 'true'
  })

  if (loading || assistBotStarted || !telegramLinked || dismissed) return null

  const handleDismiss = () => {
    setDismissed(true)
    localStorage.setItem(`${DISMISS_KEY}_${orgId}`, 'true')
  }
    ...
    return (
      <div className="flex items-center gap-2 p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm">
        ...
        <span className="flex-1 text-amber-800">
          Чтобы получать уведомления о негативе, неотвеченных вопросах и оттоке в Telegram,{' '}
          <a
        ...
        <button onClick={handleDismiss} className="text-amber-400 hover:text-amber-600 flex-shrink-0" aria-label="Закрыть">
          <X className="w-4 h-4" />
        </button>
      </div>
    )
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 pr-4">
          ...
          <p className="text-sm text-amber-700 mt-1">
            Запустите бота @{ASSIST_BOT_USERNAME}, чтобы получать уведомления о негативных обсуждениях, неотвеченных вопросах и участниках на грани оттока прямо в личные сообщения Telegram.
          </p>
          <a
            href={`https://t.me/${ASSIST_BOT_USERNAME}`}
            target="_blank"
