...
export interface VerifyCodeResult {
  ...
  error?: string
  errorCode?: string
  /** True when the code was generated by an already-logged-in user (welcome screen
   *  TG linking flow). In this case no auth link is needed — the user is already
   *  authenticated; the bot should just send a "connected" confirmation. */
...
export async function verifyTelegramAuthCode(params: VerifyCodeParams): Promise<VerifyCodeResult> {
  ...
  try {
    ...
    // 4. Resolve user:
    //    Case A — code was generated by a logged-in user (welcome screen linking flow):
    //             authCode.user_id is set → update that user's tg_user_id directly,
    //             no need to look up by telegram_user_id or create a new account.
    //    Case B — standard TG registration: find or create user by telegram_user_id.

    let userId: string
    let alreadyAuthenticated = false

    if (authCode.user_id) {
      // Case A: link TG to existing email/OAuth account
      logger.info({ user_id: authCode.user_id, telegram_user_id: telegramUserId }, 'Linking TG to existing user (welcome screen flow)');
      userId = authCode.user_id
      alreadyAuthenticated = true

      const { error: updateTgError } = await adminSupabase
        .from('users')
        .update({ tg_user_id: telegramUserId })
        .eq('id', userId)

      if (updateTgError) {
        logger.error({ user_id: userId, error: updateTgError.message }, 'Failed to update tg_user_id on user');
        return { success: false, error: 'Failed to link Telegram account', errorCode: 'UPDATE_ERROR' }
      }

      logger.info({ user_id: userId, telegram_user_id: telegramUserId }, 'tg_user_id updated on user');
    } else {
      // Case B: standard flow — find or create user by Telegram ID
      ...
      if (existingAccount) {
        ...
        if (signUpError || !newUser) {
          logger.error({ telegram_user_id: telegramUserId, error: signUpError?.message }, 'Error creating user');
          return { success: false, error: 'Failed to create user', errorCode: 'USER_CREATION_ERROR' }
        }
