...
function getEmailContent(ctx: UserContext, stepKey: string): { subject: string; html: string } {
  ...
  switch (stepKey) {
    ...
    case 'video_overview':
      return {
        subject: 'Что ещё умеет Orbo — краткий обзор',
        html: emailLayout(greeting, `
          ...
          <ul style="color:#4b5563; padding-left:20px;">
            ...
            <li><strong>Каналы</strong> — комментаторы постов тоже попадают в карточки</li>
          </ul>
          ${ctaButton(`${APP_URL}/orgs`, 'Посмотреть все возможности')}
        `),
      }
...
function getTelegramContent(ctx: UserContext, stepKey: string): string {
  ...
  switch (stepKey) {
    ...
    case 'video_overview':
      return (
        `📋 <b>${hi}что ещё можно попробовать</b>\n\n` +
        `Кроме событий, в Orbo есть:\n` +
        `• Заявки на вступление — анкета через MiniApp, spam-score\n` +
        `• Анонсы — бот публикует в группы автоматически\n` +
        `• Импорт истории чата — сразу видно, кто активен\n\n` +
        `👉 <a href="${APP_URL}/orgs">Открыть Orbo</a>`
      )
