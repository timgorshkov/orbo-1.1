...
export default function AppsPage({ params }: { params: { org: string } }) {
  ...
  const fetchApps = async () => {
    try {
      setIsLoading(true);
      console.log('[Apps Page] Fetching apps for org:', params.org);
      const response = await fetch(`/api/apps?orgId=${params.org}`);
      if (response.ok) {
      ...
      {!isLoading && apps.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {apps.map((app) => (
            <Link
              key={app.id}
              href={`/app/${params.org}/apps/${app.id}`}
