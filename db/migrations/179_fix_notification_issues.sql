-- =============================================
-- Migration 179: Fix notification issues
-- 1. Improve inactivity dedup to prevent spam
-- 2. Add group name to inactivity description
-- =============================================

-- Update get_org_notifications to include group name in description
...
-- Add column to track last activity time when inactivity notification was sent
-- This allows us to only re-notify when activity has occurred since the last notification
ALTER TABLE notification_logs 
ADD COLUMN IF NOT EXISTS last_activity_at TIMESTAMPTZ;

-- Comment
COMMENT ON COLUMN notification_logs.last_activity_at IS 'For inactivity notifications: timestamp of last activity when notification was sent. Used to prevent spam.';

-- Create index for efficient duplicate checking
CREATE INDEX IF NOT EXISTS idx_notification_logs_dedup_check
ON notification_logs(rule_id, dedup_hash, created_at DESC);
