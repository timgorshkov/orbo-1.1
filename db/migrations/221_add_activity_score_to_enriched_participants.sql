-- =============================================
-- Migration 221: Add activity_score to get_enriched_participants
-- 
-- Issue: Members page filters showed Core=0, Experienced=0 because
-- the get_enriched_participants RPC did not return activity_score.
-- Client-side categorization always defaulted to silent/other.
-- 
-- Also fixes real_last_activity: now uses GREATEST(last_message, last_activity)
-- instead of COALESCE which picks first non-null (potentially stale value).
-- =============================================

-- Must DROP first because return type changed (added activity_score column)
DROP FUNCTION IF EXISTS get_enriched_participants(UUID, BOOLEAN);
...
BEGIN
  ...
  SELECT 
    ...
    -- Activity score for engagement categorization
...
COMMENT ON FUNCTION get_enriched_participants IS 'Returns enriched participant data for an organization with message stats, tags, roles, and activity_score. Fixed: added activity_score column.';
