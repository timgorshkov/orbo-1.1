-- Migration 087: Fix Analytics RPC Functions
-- Date: Nov 5, 2025
-- Purpose: Fix issues with analytics data queries

-- Drop existing functions (signatures changed)
...
DROP FUNCTION IF EXISTS get_activity_heatmap(UUID, INT, BIGINT);  -- Original had 3 params
DROP FUNCTION IF EXISTS get_top_contributors(UUID, INT, BIGINT);

-- ============================================================================
-- 1. Fix Activity Timeline - Generate ALL days in range (even with 0 activity)
-- ============================================================================
...
COMMENT ON FUNCTION get_activity_timeline IS 'Returns daily activity timeline for ALL days in range (fills gaps with zeros)';

-- ============================================================================
-- 2. Fix Engagement Breakdown - Correct category logic + always return all 4
-- ============================================================================
...
BEGIN
  ...
  WITH participant_stats AS (
    ...
    GROUP BY p.id, p.tg_user_id, p.last_activity_at, pg.source
  ),
  categorized AS (
    SELECT 
      CASE
        ...
        WHEN first_joined_at >= NOW() - INTERVAL '30 days' 
          AND join_source IN ('telegram', 'webhook_join')
          THEN 'newcomers'
  ...
  WHERE ac.category != 'other' OR COALESCE(cc.count, 0) > 0  -- Only show 'other' if non-zero
  ORDER BY 
    CASE ac.category
      WHEN 'core' THEN 1
...
COMMENT ON FUNCTION get_engagement_breakdown IS 'Returns engagement category breakdown (always includes core/experienced/silent/newcomers)';

-- ============================================================================
-- 3. Fix Reactions & Replies - Ensure proper field names match API
-- ============================================================================
...
COMMENT ON FUNCTION get_reactions_replies_stats IS 'Returns reactions and replies stats with comparison to previous period';

-- ============================================================================
-- 4. Fix Activity Heatmap - Swap rows/columns (day_of_week as columns, hour as rows)
-- ============================================================================
...
COMMENT ON FUNCTION get_activity_heatmap IS 'Returns activity heatmap by hour and day of week';

-- ============================================================================
-- 5. Fix Top Contributors - Add participant full_name from participants table
-- ============================================================================
...
COMMENT ON FUNCTION get_top_contributors IS 'Returns top contributors with full participant data';
...
DO $$ BEGIN RAISE NOTICE 'Migration 087 Complete: Analytics functions fixed.'; END $$;
