-- Migration 129: Fix jsonb_object_keys() usage in update_participant_from_registration_data
-- Date: Nov 19, 2025
-- Purpose: jsonb_object_keys() returns SET OF TEXT, cannot be used in OR condition
--          This causes "argument of OR must not return a set" error
...
BEGIN
  ...
  -- FIX: Don't use jsonb_object_keys() in OR - it returns a SET
...
COMMENT ON FUNCTION update_participant_from_registration_data IS 'Updates participant profile from registration data. Uses SECURITY DEFINER and EXECUTE to bypass RLS. Fixed: replaced jsonb_object_keys() with simple empty object check.';

DO $$ BEGIN RAISE NOTICE 'Migration 129 Complete: Fixed jsonb_object_keys() usage in update_participant_from_registration_data.'; END $$;
