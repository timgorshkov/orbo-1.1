-- Migration 131: Fix phone -> phone_number in update_participant_from_registration_data trigger
-- Date: Nov 20, 2025
-- Purpose: Update trigger to use phone_number instead of phone field
...
BEGIN
  ...
  EXECUTE format('SELECT full_name, email, phone_number, bio, custom_attributes FROM %I.participants WHERE id = $1', 'public')
    INTO participant_full_name, participant_email, participant_phone_number, participant_bio, participant_custom_attrs
  ...
  LOOP
    ...
    CASE field_mapping_var
      ...
      WHEN 'phone_number' THEN
        -- Only update if participant.phone_number is empty
        IF participant_phone_number IS NULL OR participant_phone_number = '' THEN
          RAISE NOTICE '[TRIGGER update_participant] Updating phone_number to %', field_value;
          EXECUTE format('UPDATE %I.participants SET phone_number = $1 WHERE id = $2', 'public')
            USING field_value, NEW.participant_id;
        END IF;
...
COMMENT ON FUNCTION update_participant_from_registration_data IS 'Updates participant profile from registration data. Uses SECURITY DEFINER and EXECUTE to bypass RLS. Only updates fields if they are NULL or empty (does not overwrite existing values). Fixed: phone -> phone_number.';

DO $$ BEGIN RAISE NOTICE 'Migration 131 Complete: Fixed phone -> phone_number in update_participant_from_registration_data.'; END $$;
