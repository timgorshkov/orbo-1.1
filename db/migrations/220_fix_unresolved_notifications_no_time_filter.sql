-- =============================================
-- Migration 220: Fix get_org_notifications to show all unresolved alerts
-- 
-- Issue: Unresolved AI alerts (notification_logs) older than 7 days
-- were hidden from the Notifications page, but still showed on dashboard.
-- 
-- Fix: Don't apply time window to unresolved notification_logs.
-- Time window only applies to resolved items (to limit history).
-- =============================================
...
CREATE OR REPLACE FUNCTION get_org_notifications(
  ...
  p_hours_back INTEGER DEFAULT 168 -- 7 days (only for resolved notification_logs)
)
RETURNS TABLE (
  id UUID,
...
BEGIN
  ...
  -- Unresolved: always show (no time window)
  -- Resolved: only within p_hours_back window
  SELECT 
    nl.id,
    nl.created_at,
  ...
  WHERE nl.org_id = p_org_id
    ...
    AND (
      nl.resolved_at IS NULL  -- Unresolved: always show
      OR nl.created_at > NOW() - (p_hours_back || ' hours')::INTERVAL  -- Resolved: within time window
    )
