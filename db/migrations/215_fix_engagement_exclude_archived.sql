-- Migration 215: Fix engagement to exclude participant_status='excluded'
-- Date: Jan 26, 2026
-- Purpose: participant_status='excluded' contains archived participants, not status='archived'
...
BEGIN
  -- Get total participants in this org (exclude excluded and merged)
  ...
  -- Calculate real activity dates for each participant using JOINs (exclude excluded)
...
DO $$ BEGIN RAISE NOTICE 'Migration 215 Complete: Fixed engagement to exclude participant_status=excluded'; END $$;
