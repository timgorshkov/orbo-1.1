-- Migration 127: Fix update_participant_from_registration_data trigger to bypass RLS
-- Date: Nov 19, 2025
-- Purpose: This trigger executes AFTER INSERT and does SELECT/UPDATE on participants and event_registration_fields
--          These tables have RLS policies with OR causing "argument of OR must not return a set"
...
COMMENT ON FUNCTION update_participant_from_registration_data IS 'Updates participant profile from registration data. Uses SECURITY DEFINER and EXECUTE to bypass RLS.';

DO $$ BEGIN RAISE NOTICE 'Migration 127 Complete: Fixed update_participant_from_registration_data trigger to bypass RLS.'; END $$;
