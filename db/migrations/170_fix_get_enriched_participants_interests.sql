-- Migration 170: Fix get_enriched_participants - handle missing interests column
-- The interests column may not exist in participants table

-- First, ensure interests column exists (with COALESCE for safety)
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE participants ADD COLUMN interests TEXT[] DEFAULT '{}';
  END IF;
END $$;

-- Recreate the function with safer column access
